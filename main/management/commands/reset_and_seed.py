...
from main.models import (
    Lesson, ScheduleTemplate, StudentPerformance, LessonAttachment,
    HomeworkSubmission, SubmissionAttachment, PrivateComment,
    StudyGroup, TeachingAssignment, EvaluationType,
    Classroom, User, AbsenceReason,
)
...
SCHEDULE_SLOTS = [
    (1, 1, 0), (1, 2, 1), (1, 3, 2), (1, 4, 3),   # Пн – 4 пари
    (2, 1, 4), (2, 2, 5), (2, 3, 6), (2, 4, 0),   # Вт – 4 пари
    (3, 1, 1), (3, 2, 2), (3, 3, 3),               # Ср – 3 пари
    (4, 1, 4), (4, 2, 5), (4, 3, 6), (4, 4, 1),   # Чт – 4 пари
    (5, 1, 0), (5, 2, 2), (5, 3, 3),               # Пт – 3 пари
]
...
def _hw_practical(topic):
    deadline = (date.today() + timedelta(days=random.choice([3, 5, 7]))).strftime('%d.%m.%Y')
    options = [
        (
...
def _hw_lab(topic):
    deadline = (date.today() + timedelta(days=random.choice([5, 7, 10]))).strftime('%d.%m.%Y')
    options = [
        (
...
class Command(BaseCommand):
    ...
    def _create_templates(self):
        ...
        for group in StudyGroup.objects.filter(is_active=True).order_by('name'):
            ...
            self.stdout.write(
                f'  {group.name}: {len(group_templates)} templates '
                f'({n} subjects)'
            )
        ...
        for lesson in lessons:
            ...
            for student in students:
                if random.random() < 0.10 and absence_reasons:
                    ...
                    perf_to_create.append(StudentPerformance(
                        lesson=lesson,
                        student=student,
                        absence=absence,
                        earned_points=None,
